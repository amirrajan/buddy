...
module Buddy
  ...
  def iframes_accessible?
    has_blank_iframes = @browser.frames.any? { |f| f.url == "about:blank" }
  ...
  def elements selector, timeout: default_timeout
    elements_css(selector, timeout: timeout) || elements_xpath(selector, timeout: timeout)
  end
